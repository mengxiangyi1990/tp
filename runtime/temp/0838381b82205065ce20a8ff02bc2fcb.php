<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:79:"D:\phpStudy\WWW\twothink\public/../application/home/view/default/sale\show.html";i:1507443544;}*/ ?>
...
<div class="main">
    ...
    <div class="container-fluid">
        ...
        <div class="noticeDetailInfo">发布人:<?php echo $list['author']; ?></div>
        <div class="noticeDetailInfo">发布时间：<?php echo time_format($list['create_time'] ); ?></div>
        <div class="noticeDetailInfo">价格：<?php echo $list['price']; ?><?php echo !empty($list['unit']) && $list['unit']==2?"元/月":"万元"; ?></div>
    ...
    <div id="think_page_trace_tab" style="display: none;background:white;margin:0;height: 250px;">
        ...
        <div id="think_page_trace_tab_cont" style="overflow:auto;height:212px;padding: 0; line-height: 24px">
            ...
            <div style="display:none;">
                <ol style="padding: 0; margin:0">
                    <li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">SHOW COLUMNS FROM `channel` [ RunTime:0.0004s ]</li><li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">SELECT * FROM `channel` WHERE `pid` = 0 ORDER BY `sort` asc  [ RunTime:0.0001s ]</li><li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">SELECT `id` FROM `category` WHERE `name` IN ('notice','service','shop','activity')  [ RunTime:0.0001s ]</li><li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">UPDATE `document` SET `status`='-1' WHERE `category_id` IN ('42','43','44','45') AND `deadline` &lt; 1507131389 [ RunTime:0.0002s ]</li><li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">SELECT `type`,`name`,`value` FROM `config` WHERE `status` = 1  [ RunTime:0.0001s ]</li><li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">SELECT * FROM `channel` WHERE `pid` = 0 ORDER BY `sort` asc  [ RunTime:0.0001s ]</li><li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">SHOW COLUMNS FROM `document` [ RunTime:0.0005s ]</li><li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">SELECT me.nickname,d.*,da.* FROM document d INNER JOIN document_article as da on d.id=da.id INNER JOIN member as me on d.uid=me.uid  WHERE d.id = '55' LIMIT 1   [ RunTime:0.0003s ]</li><li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">UPDATE `document` SET `view`=view+1 WHERE `id` = 55 [ RunTime:0.0001s ]</li>    </ol>
            </div>
            <div style="display:none;">
                <ol style="padding: 0; margin:0">
                </ol>
